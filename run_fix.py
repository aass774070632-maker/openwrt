...
# Replace the loop via regex
loop_regex = r"uci\.sections\('wireless', 'wifi-iface'\)\.forEach\(function\(section\) \{[\s\S]*?uci\.set\('wireless', sid, 'disabled', '1'\);\s*\}\s*\}\);"
...
if re.search(loop_regex, code):
    code = re.sub(loop_regex, new_loop, code)
    print("Loop replaced.")
else:
