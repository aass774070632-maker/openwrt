...
### W) Phase-4: Fix Ghost SSIDs, Mode Switching, and Safe Hotspot Cleanup (2026-05-16)

Objective:
1. When switching from Hotspot to non-Hotspot modes (or stopping Hotspot), ensure that regular LAN and user settings (IPs, Firewall rules, Subnets) are not accidentally deleted by the backend cleanup script.
2. Fix the "Ghost SSID" problem where leftover Wi-Fi interfaces from previous mode selections remained active because they were disabled instead of cleanly removed.

Root Cause Analysis for Non-Hotspot Disruption:
- The `/usr/libexec/alemprator-setup/cleanup-hotspot` backend script aggressively pattern-matched `*hotspot*`, `*192.168.10*`, and `*192.168.20*` across DHCP, Wireless, and Firewall UCI configurations.
- If a user had a standard LAN subnet on `192.168.10.x` or a custom port-forwarding rule named `my-hotspot-cam`, the script would destructively wipe it when disabling Hotspot Quick mode.

Root Cause Analysis for Ghost SSIDs:
- The LuCI `setup.js` wizard used `uci.set('wireless', sid, 'disabled', '1')` on overlapping interfaces instead of safely removing unmanaged sections. This caused leftover SSIDs to pile up as disabled, sometimes resurrecting and causing confusion.

Solutions Implemented and Validated:
1. **Frontend `setup.js` Ghost SSID Wipe**:
   - Introduced `managedSids = {}` dictionary tracking in JavaScript.
   - During the Wizard's configuration loop, any actively built wireless interface (LAN, Mesh, Hotspot) is flagged as `managed`.
   - After the loop, the code safely enumerates UCI sections and calls `uci.remove('wireless', sid)` for any interface not explicitly mapped in `managedSids`.

2. **Backend `/usr/libexec/alemprator-setup/cleanup-hotspot` Sandboxing**:
   - Refactored `cleanup-hotspot` to strictly check for space-padded `" hotspot "` and `" hotspot2 "` interfaces instead of loose string matching.
   - Removed `*192.168.10*` and `*192.168.20*` from the DHCP removal lines; the script now exclusively deletes Chilli's `/hotspot` URI options via strictly matched paths.
   - Strict pattern matching restored on firewall rules: `hotspot_openwrt_*|*" hotspot "*|*" hotspot2 "*)`. This isolates the cleanup scope exclusively to configurations generated by the hotspot portal logic, guaranteeing zero destruction to user LAN, VLAN, and generic port forwards.

### X) Further Cleanup and Bugfixes found on May 22, 2026

During an extended sweep of configuration removal flaws, additional bugs where the system arbitrarily deleted configurations when switching modes were identified and patched.

1. **Protective Measures for `dnsmasq` Global Configuration (`package/luci-app-hotspot-openwrt/files/usr/libexec/hotspot-openwrt/apply`)**:
   - The Hotspot apply script was blindly executing `uci -q delete dhcp.@dnsmasq[0].confdir` to force dnsmasq to ignore extra configurations and instead rely on Hotspot's own rules. 
   - **Root Cause & Impact**: Many built-in OpenWrt packages (e.g. Adblock) rely on `confdir`. Deleting the `confdir` instruction entirely will break configurations for those tools.
   - **Fix**: Adjusted the script to no longer destructively wipe `confdir`. Instead, it checks if `confdir` is set. If not, it natively defaults to `/tmp/dnsmasq.d` instead of explicitly un-setting everything. The `addnhosts` wipe remains untouched as that is specifically safe-checked for captive DNS filtering.

2. **Chilli Aggressive Process Wipe Fixed (`package/luci-app-setup/files/usr/libexec/alemprator-setup/cleanup-hotspot`)**:
   - Similar to the Firewall wipe issue, the backend `cleanup-hotspot` script was aggressively pattern matching `*192.168.10*` and `*192.168.20*` within *any* Chilli daemon configs (`tundev`, `dhcpif`, `network`, `uamlisten`, `dynip` etc).
   - **Root Cause & Impact**: A user running CoovaChilli for entirely different purposes not linked to the ALEMPRATOR wizard on these generic subnets would find their configuration deleted every time they passed through the WiFi setup page.
   - **Fix**: Replaced the global `*192.168.10*` matching line with strict constraints. The Chilli cleanup now explicitly looks at `$dhcpif:$network` specifically for the values `*hotspot_openwrt*`, `" hotspot "`, or `" hotspot2 "`.

3. **Validation Outcome**:
   - With these fixes applied on top of Phase 4, turning off Hotspot Quick mode, altering subnets, or disabling Hotspot from the Setup Wizard will strictly touch **ONLY** configurations injected by the Hotspot module. Non-Hotspot features (LAN subnets, external `dnsmasq` additions, custom port forwards) will survive unmodified.

3. **Protection against 'old_secondary_subscriber_interface' manual overrides (`package/luci-app-hotspot-openwrt/files/usr/libexec/hotspot-openwrt/apply`)**:
   - The Hotspot apply script contained logic to delete old secondary interfaces and networks when switching out of dual mode.
   - **Root Cause & Impact**: If a user previously manually forced the internal variable `quick_subscriber_interface_secondary` to `lan` or `guest` while testing or attempting to hack a setup, and then toggled dual-mode off, the script would execute `uci -q delete network.lan`, completely deleting the main LAN.
   - **Fix**: Wrapped the deletion in a strict `case "$old_secondary_subscriber_interface" in hotspot*)` safety guard. Now, if the old interface name doesn't start with `hotspot`, the script will refuse to delete it, sparing the user's manual networking setup from unexpected wipeouts.

4. **Corrected Stubborn DHCP Generic Matching (`package/luci-app-setup/files/usr/libexec/alemprator-setup/cleanup-hotspot`)**:
   - Discovered that the Phase 4 fix for DHCP options (`*192.168.10*` deletions) failed to apply due to unescaped Python regex errors in the original patch.
   - **Root Cause & Impact**: The `cleanup-hotspot` script was still aggressively matching and destroying user DHCP scopes containing `.10.` and `.20.`.
   - **Fix**: Re-ran the patch manually as an exact text swap. The `cleanup-hotspot` script now correctly and strictly only looks for `*/hotspot*)` in DHCP options 114 before triggering a deletion.
