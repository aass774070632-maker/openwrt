...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		if( this.config.hideUrl ){
			str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="javascript:' + this.obj + '.href(' + nodeId + ');"';
		}else{
...
//anchor href - HJKIM 2009.05.25
//for hide href url
dTree.prototype.href = function(id) {
	if (!this.config.useSelection) return;
	var cn = this.aNodes[id];
	if (cn._hc && !this.config.folderLinks) return;
