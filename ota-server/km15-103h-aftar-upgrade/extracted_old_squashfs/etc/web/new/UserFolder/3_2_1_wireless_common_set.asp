...
function onAutoChannelRange(){
	...
	if( channelMode == '-1' ){			
		...
		if( gFreqType == '1' ){
			...
			if( !(gAutoChannelRange & (1<<30)) ||	//auto -> auto(select)	
				wirelessMode != gOrgWirelessMode ||	//change wireless mode
				varChBandWidth!= gOrgBandWidth){	//change bandwidth
				needReset = 1;
			}
			if( needReset ){
				if( varChBandWidth == "3" ) //160M
					autoChannelRange = 0x000ffff;
				else if( varChBandWidth == "18" ) //80+80
					autoChannelRange = 0x0f0000f; //80+80(nc)
				else if( varChBandWidth == "2" )	//80
					autoChannelRange = 0x0f0000f;
				else if( varChBandWidth == "1" )	//40
					autoChannelRange = 0x0f0000f;
				else								//20
		...
		if( gFreqType == '1' ){
			//2.4G
			setAutoChannelRangeValue(gFreqType, 0x07ff);	//1~11
		}else{
			//5G
			setAutoChannelRangeValue(gFreqType, 0x787fff);	//dfs 영역선택
		}
		$("#viewChannelSelect").hide();
		$("#viewChannelManual").hide();
	}
...
//		shortSlot = '<% mcr_getCfgWireless("Wlan_ShortSlot", gWlanIfIndexEJ); %>';
		...
		qam = '<%mcr_getCfgWireless("Wlan_VHT_Use256QAM", gWlanIfIndexEJ); %>';
...
/*
	if(mmimo != "0"){
		document.getElementsByName("mmimo")[0].checked = true;
		$("#view_mmimo_list").show();
		if(mmimo == "1"){
			document.getElementsByName("mmimo_list")[0].checked = true;
		}else if(mmimo == "2"){
			document.getElementsByName("mmimo_list")[1].checked = true;
		}else if(mmimo == "3"){
			document.getElementsByName("mmimo_list")[2].checked = true;
		}
	}else{
		$("#view_mmimo_list").hide();
		document.getElementsByName("mmimo")[1].checked = true;
	}
*/
	if(qam != "0"){
		document.getElementsByName("qam")[0].checked = true;
			
	}else{
		document.getElementsByName("qam")[1].checked = true;
	}
...
function changeTxBF(val){
	var txbf = '<% mcr_getCfgWireless("Wlan_TxBeamforming", gWlanIfIndexEJ); %>';
	if(val == "1"){
		$("#view_txbf_list").hide();
		if(txbf == "0"){
			document.getElementsByName("txbf_list")[2].checked = true;
							...
							<tr>
								<td>
									<table class="TB" width="100%" border="0">
										...
										<tr id="view_wireless_mode_5G_1" style="display:none">
											<td class="BG2" style="width:140px" nowrap>무선 모드</td>
											<td class="BG2-2" width="600" colspan="4">
												<table border="0" cellpadding="0" cellspacing="0" class="font1">
													<tr>
														<td width="100">
															<input type="radio" name="uiWirelessMode_5G_1" value="8">
															<label>a</label>
														</td>
														<td width="100">
															<input type="radio" name="uiWirelessMode_5G_1" value="4">
															<label>n</label>
														</td>
														<td width="100">
															<input type="radio" name="uiWirelessMode_5G_1" value="16">
															<label>ac</label>
														</td>
														<td width="100">
															<input type="radio" name="uiWirelessMode_5G_1" value="12">
															<label>an</label>
														</td>
														<td width="100">
															<input type="radio" name="uiWirelessMode_5G_1" value="20">
															<label>n+ac</label>
														</td>
														<td>
															<input type="radio" name="uiWirelessMode_5G_1" value="28">
															<label>an+ac</label>
														</td>
													</tr>
												</table>
											...
											<td class="BG2-2" width="600" colspan="4">
												<table border="0" cellpadding="0" cellspacing="0" class="font1">
													...
													<tr id="view_txbf_list">
														<td width="110">
															<input type="radio" name="txbf_list" value="1">Explicit TxBF
														</td>
														<td width="110">
															<input type="radio" name="txbf_list" value="2">Implicit TxBF
														</td>
														<td width="220">
															<input type="radio" name="txbf_list" value="3">Explicit/Implicit TxBF
														</td>
													</tr>
												</table>
											</td>
										...
										<tr id="view_qam" style="display:none">
											<td class="BG2" style="width:140px;">256QAM</td>
											<td class="BG2-2" width="600" colspan="4">
												<table border="0" cellpadding="0" cellspacing="0" class="font1">
													<tr>
														<td width="110">
															<input type="radio" name="qam" value="1">활성
														</td>
														<td>
															<input type="radio" name="qam" value="0">비활성
														</td>
													</tr>
												</table>
											</td>
