...
// mesh enable && master/slave 일 경우 wpa2 wpa-psk aes만 설정 되도록 설정
...
function initForm_WLAN(flag){
	...
	$("#wlanRedirectPage").val("/new/UserFolder/2_simple_open_set.asp");
	$("#wlanViewWMM").hide();
...
*/
	...
	$("#wlanBtnSecurity").bind( "click", function(){
		...
		if( validateOnSubmit_WLAN_SecurityType(null) == true ){
			var ret;
			var vapEnable = 0;
			var wlanSSIDIdx = gWlanIfIndex;
			var Activity = '<% mcr_getCfgWireless("Wlan_Enable", 4); %>';
			if( $("input[name='wlanRadioActivity_nespot']:checked").val() =='1' ||
				$("input[name='wlanRadioActivity_ollehWiFi']:checked").val() =='1' ){
				vapEnable = 1;
			}
			...
			if(ret == true && wlanSSIDIdx == 0){
				if(Activity == 1 && form_simple.wlanRadioActivity[1].checked == true){
					ret = confirm("IoW WLAN 접속 설정이 비활성화 됩니다. 계속하시겠습니까?");
				}else if(Activity == 0 && form_simple.wlanRadioActivity[0].checked == true){
			...
			if( ret == true ){
				...
				$('#form_simple').attr("action", "/goform/mcr_KT_setWirelessSecurity").submit();
			}
		}
		return false;
	});	
	...
	if ( document.form_simple.macCloneEnbl[0].checked == true)
	{
		if (document.form_simple.macCloneMac.value != "") {
			...
			if (re.test(document.form_simple.macCloneMac.value)) { 
				document.form_simple.redirect_url.value = "/new/UserFolder/2_simple_open_set.asp"
				form_act(url);
...
function validateOnSubmit_IPAlloc()
{
	...
	var waninterface = '<% mcr_getCfgWireless("SysOperMode_WanInterface", "-1"); %>';
        if ( document.form_simple.macCloneEnbl[0].checked == true)
        {
                if (document.form_simple.macCloneMac.value != "") {
                        var re = /[A-Fa-f0-9]{2}:[A-Fa-f0-9]{2}:[A-Fa-f0-9]{2}:[A-Fa-f0-9]{2}:[A-Fa-f0-9]{2}:[A-Fa-f0-9]{2}/;
		...
		if(waninterface == "1") {
			...
			if (org_opmode != 2) {
				...
				if(NetKeepEn == "1"){
					...
					/*	if(wlanRadioActivity == '1' && MeshDeviceRole == '1'){
						confirmed = confirm("브릿지모드 설정 시, Mesh 설정은 사용할 수 없습니다. 계속하시겠습니까?");
						if(!confirmed)
						return false;
						}
					 */
...
<input type="hidden" name=SETIPMODE value="/new/UserFolder/2_simple_open_set.asp">
<input type="hidden" name="redirect_url" id="redirect_url" value="/new/UserFolder/3_7_8_system_restart_process.asp">
<input type="hidden" name="redirect_admWanSet" id="redirect_admWanSet" value="/new/UserFolder/2_simple_open_set.asp">
...
<input type="hidden" id="security_flag" name="security_flag" value="0">
