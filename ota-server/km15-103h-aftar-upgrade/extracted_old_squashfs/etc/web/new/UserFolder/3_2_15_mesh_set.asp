...
function form_act(url)
{
	...
	if(url == "/goform/mcr_setWirelessMeshPushRoaming"){
		if(!CheckValue())
			return false;
	}else if(url == "/goform/mcr_setWirelessMeshPush"){
		if(form_wlanMesh.wlanMeshActivity[0].checked == true){
			if(mesh_popup()){
				if(wlanSecurityMode_mesh != '0' ) {
					if(num < 0 || eng < 0 || value.length < 10){
						alert("암호는 영문 대소문자와 숫자 조합으로 10자 이상이어야 합니다");
	...
	if(form_wlanMesh.wlanMeshActivity[1].checked == true){
		parent.mcrProgress.startProgressSimple('apply', 60);
	}else{
		parent.mcrProgress.startProgressSimple('apply', 60);
	}
...
function initTable(){
	...
	tableRule.addColumn(MCRColumn.TYPE_NORMAL, "MAC 주소", "width='40%'", strTableTh, strTableTd_nopadding+" align='center'", "");
...
<input type="hidden" id="wlanRedirectPage" name="wlanRedirectPage" value="/new/UserFolder/3_2_15_mesh_set.asp">
					...
					<td valign="top">
						<table width="98%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr id="apply_btn">
								<td class="PD6" colspan="3">
									<input type="image" src="/images/BTN/BTN_01.gif" value="Apply" id="btn_apply" name="btn_apply" height="24" width="52" onclick="form_act('/goform/mcr_setWirelessMeshPush'); return false;"></input>
								</td>
							</tr>
						</table>
						<table width="98%" border="0" cellspacing="0" cellpadding="0">
						...
						<table width="98%" border="0" cellspacing="0" cellpadding="0">
							<tr id="view_list" style="display:none;">
								<td width="670">
									<div>
										<table class="TB TB-1" id="Grid_Table" width="100%" border="0" bgcolor="#FFFFFF">
											<tr height="20">
						...
						<table width="98%" border="0" cellspacing="0" cellpadding="0">
							<tr height="120" id="view_list1" style="display:none">
								<td width="670" valign="top">
									...
									<div id="view_aplist" style="height:120; width:760; overflow:-moz-scrollbars-vertical; overflow-x:no; overflow-y:auto;"></div>
