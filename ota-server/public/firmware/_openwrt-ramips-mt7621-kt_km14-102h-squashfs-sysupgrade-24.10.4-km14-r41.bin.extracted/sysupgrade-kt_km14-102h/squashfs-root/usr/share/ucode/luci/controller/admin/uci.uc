...
function uci_apply(rollback) {
	if (rollback) {
		...
		let applied = ubus_call('uci', 'apply', {
			ubus_rpc_session: ctx.authsession,
			rollback: false
		});

		if (applied)
			system("/bin/sh -c 'if [ -x /etc/init.d/alemprator-firstboot ]; then /etc/init.d/alemprator-firstboot force_cleanup >/dev/null 2>&1 || true; fi'");
...
return {
	...
	action_confirm: function() {
		if (uci_confirm(http.formvalue('token')))
			system("/bin/sh -c 'if [ -x /etc/init.d/alemprator-firstboot ]; then /etc/init.d/alemprator-firstboot force_cleanup >/dev/null 2>&1 || true; fi'");
		ubus_state_to_http(last_ubus_error);
	},
